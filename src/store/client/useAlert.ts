...
const useAlertStore = create<AlertState>((set) => ({
  ...
  handleClose: () => {
    set((state) => {
      if ((state.alertOptions?.catchOnCancel ?? false) && state.awaitingPromiseRef != null) {
  ...
  setLoadingAlert: (loading: boolean) =>
    set((state) => ({
      alertOptions: state.alertOptions != null ? { ...state.alertOptions, isLoading: loading } : null
    }))
}))

export const useAlert = () => {
  const { alertOptions, alert, handleClose, handleSubmit, setLoadingAlert } = useAlertStore((state) => ({
